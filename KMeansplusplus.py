...
def load_dataset(filename):
    ...
    try:
        ...
        # Setting the first column as 'label' as contains the labels.
...
def initialSelection(data, k):
    """
    Selects initial centroids using the KMeans++ algorithm.
    ...
    Returns:
    - pandas.DataFrame: A DataFrame containing k initially selected centroids.
    """
    # Randomly selecting the first centroid from the data.
    centroids = [data.iloc[np.random.randint(0, len(data))]]

    for _ in range(1, k):
        # Computing the distances from the nearest centroid.
        distances = np.array([min([computeDistance(data.iloc[i], centroid) for centroid in centroids]) for i in range(len(data))])
        
        # Choosing the next centroid with a probability proportional to the square of the distance.
        ...
        for i, p in enumerate(cumulative_probabilities):
            if r < p:
                next_centroid = data.iloc[i]
                centroids.append(next_centroid)
                break

    # Returns centroids as a new DataFrame.
    newDf = pd.DataFrame(centroids).reset_index(drop=True)
...
def kMeansPlusPlus(data, k, maxIter=100):
    """
...
def computeSilhouetteScores(data, kRange):
    ...
    for k in kRange:
        ...
        _, clusterIds = kMeansPlusPlus(data, k)
...
def plot_silhouette(kRange, silhouetteScores):
    ...
    plt.savefig('kmeans++_silhouette_scores.png', format='png')

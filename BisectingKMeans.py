...
def load_dataset(filename):
    ...
    try:
        ...
        # Removing the label column and retains only features.
...
def computeSumOfSquare(df, centroid):
    """
    Computes the sum of squared distances of all points in a DataFrame from their centroid.

    Args:
    - df (pandas.DataFrame): DataFrame containing the data points of the cluster.
    - centroid (pandas.Series): The centroid of the cluster.

    Returns:
    - float: The sum of squared distances of all points in the cluster from the centroid.
    """
    sumOfSquare = np.sum((df - centroid) ** 2, axis=0).sum()
...
def computeDistance(point1, point2):
    ...
    squaredDistance = np.sum(squaredDiff, axis=0)
...
def initialSelection(data, k):
    ...
    """
    # Randomly selecting k samples from the input data and resetting the index of the resulting DataFrame.
...
def bisectingKMeans(data, k, maxIter=100):
    """
    Performs Bisecting K-Means clustering algorithm.

    Args:
    - data (pandas.DataFrame): The input DataFrame containing the data points.
    - k (int): The target number of clusters.
    - maxIter (int): The maximum number of iterations for K-Means in each bisecting step.

    Returns:
    - pandas.DataFrame: A DataFrame containing the final centroids.
    - pandas.Series: A Series containing the cluster IDs for each data point.
    """
    # Initializing the list of clusters with the entire dataset.
    clusters = [data.copy()]

    # Repeating until the desired number of clusters is obtained.
    while len(clusters) < k:
        # Find the cluster with the highest sum of squared distances.
        sumOfSquaredDistances = [computeSumOfSquare(cluster, cluster.mean()) for cluster in clusters]
        index_to_split = np.argmax(sumOfSquaredDistances)
        cluster_to_split = clusters.pop(index_to_split)

        # Applying K-Means to the cluster to be split with k=2.
        _, clusterIds = kMeans(cluster_to_split, 2, maxIter)

        # Splitting the cluster into two new clusters based on the K-Means results.
        new_clusters = [cluster_to_split[clusterIds == i] for i in range(2)]

        # Adding the two new clusters to the list of clusters.
        clusters.extend(new_clusters)

    # Assigning final cluster IDs to each data point.
    final_clusters = pd.Series(np.zeros(len(data)), index=data.index)
    for i, cluster in enumerate(clusters):
        final_clusters[cluster.index] = i
...
def computeSilhouetteScores(data, kRange):
    ...
    for k in kRange:
        ...
        _, clusterLabels = bisectingKMeans(data, k)
        
        clusterIds = clusterLabels
...
def plot_silhouette(kRange, silhouetteScores):
    ...
    plt.savefig('bisectingkmeans_silhouette_scores.png', format='png')
